...
export default function Skills() {
  ...
  useEffect(() => {
    function handleKeyDown(event) {
      ...
      const currentIndex = TECHS.findIndex((tech) => tech.title === selected);
      const nextIndex = (currentIndex + 1) % TECHS.length;
      setSelected(TECHS[nextIndex].title);
    }
  ...
  return (
    ...
    >
      <div className="w-full lg:w-1/2 flex flex-col">
        ...
        <div className="hidden md:block relative w-full mt-20 h-[400px]">
      ...
      {/* Phone */}
      <div className="flex-shrink-0 w-full max-w-[360px] h-[550px] sm:h-[600px] rounded-[40px] bg-slate-950 border-4 border-slate-800 shadow-2xl flex flex-col p-4 justify-between lg:mr-8">
        ...
        <div className="grid grid-cols-3 gap-2 px-1">
          {TECHS.map((tech) => (
            <button
              ...
              className={`h-16 rounded-xl p-1 border flex flex-col items-center justify-center transition-all duration-300 ${selected === tech.title
